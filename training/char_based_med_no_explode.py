...
def main():
    train_data = read_data_char_based(gt_path=PATH_PREFIX + 'data/datasets/1blm/1blm.train.gt',
                      noise_path=PATH_PREFIX + 'data/datasets/1blm/1blm.train.noise')
    val_data = read_data_char_based(gt_path=PATH_PREFIX + 'data/datasets/1blm/1blm.test.gt',
    ...
    model_name = 'char-based-med_no_explode'
    batch_size = 64
    num_epochs = 10
    st_epoch = 0
    ...
    train_model(model, tokenizer, optimizer, scheduler, train_data, val_data, batch_size, print_n_batches, num_epochs,
                st_epoch, model_name, CharBasedTransformer, device, save_model=not test_mode,
                use_tensorboard=not test_mode, model_version=model_version, test_mode=test_mode)
