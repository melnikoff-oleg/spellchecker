...
from grazie.spell.main.model.spellcheck_model import SpellCheckModelBase, CharBasedTransformer
...
if __name__ == '__main__':
    ...
    # med 5
    d_model = 128
    checkpoint = 'training/model_small_2_4.pt'
    model = CharBasedTransformer(config={'d_model': d_model, 'encoder_layers': 6, 'decoder_layers': 6,
                                         'encoder_attention_heads': 8, 'decoder_attention_heads': 8,
                                         'encoder_ffn_dim': d_model * 4, 'decoder_ffn_dim': d_model * 4},
                                 checkpoint=path_prefix + checkpoint)
    ...
    evaluate(model, texts_gt, texts_noise, path_prefix + 'data/experiments/char_based_transformer_med_5_epochs/')

    # med 10
    d_model = 128
    checkpoint = 'training/model_small_2_4_9.pt'
    model = CharBasedTransformer(config={'d_model': d_model, 'encoder_layers': 6, 'decoder_layers': 6,
                                         'encoder_attention_heads': 8, 'decoder_attention_heads': 8,
                                         'encoder_ffn_dim': d_model * 4, 'decoder_ffn_dim': d_model * 4},
                                 checkpoint=path_prefix + checkpoint)
    texts_gt, texts_noise = get_texts_from_file(path_prefix + 'data/datasets/bea/bea500.gt'), \
                            get_texts_from_file(path_prefix + 'data/datasets/bea/bea500.noise')
    evaluate(model, texts_gt, texts_noise, path_prefix + 'data/experiments/char_based_transformer_med_10_epochs/')

    # big 5
    d_model = 256
    checkpoint = 'training/model_big_0_4.pt'
    model = CharBasedTransformer(config={'d_model': d_model, 'encoder_layers': 6, 'decoder_layers': 6,
                                         'encoder_attention_heads': 8, 'decoder_attention_heads': 8,
                                         'encoder_ffn_dim': d_model * 4, 'decoder_ffn_dim': d_model * 4},
                                 checkpoint=path_prefix + checkpoint)
    texts_gt, texts_noise = get_texts_from_file(path_prefix + 'data/datasets/bea/bea500.gt'), \
                            get_texts_from_file(path_prefix + 'data/datasets/bea/bea500.noise')
    evaluate(model, texts_gt, texts_noise, path_prefix + 'data/experiments/char_based_transformer_big_5_epochs/')
    ...
    texts_gt, texts_noise = get_texts_from_file(path_prefix + 'data/datasets/bea/bea500.gt'), \
                            get_texts_from_file(path_prefix + 'data/datasets/bea/bea500.noise')
    evaluate(model, texts_gt, texts_noise, path_prefix + 'data/experiments/char_based_transformer_big_10_epochs/')
